...
int Process::msg_handle()
{
	...
	switch(msg_data[0])
	{
		...
		case 0x02:err=link_test();break;
		case 0x03:err=talkback();break;
...
int Process::link_test()
{
	...
	err=write(client_fd,box_data,box_data_len);
	if(err==-1)
	{
		perror("ERROR:child write to client error\n");
		msg_back_data[0]=0x01;
	...
	for(i=0;i<err;i++)
	{
		printf("%02X ",box_data[i]);
	}
	printf("\n");

	printf("child %d reading...\n",client_fd);
	err=read(client_fd,box_back_data,box_back_data_len);
	if(err==-1)
	{
		printf("ERROR:child read from %d error\n",client_fd);
		msg_back_data[0]=0x01;
	...
	printf("process linktest read from box:");
	for(i=0;i<err;i++)
	{
		printf("%02X ",box_back_data[i]);
	}
	printf("\n");
	...
	msg_back_data[4]=msg_back_data[3]*10+(box_back_data[8]-0x30);//box version
