...
int Process::link_test()
{
	...
	msg_back_data[3]=msg_back_data[3]*0x10+(box_back_data[6]-0x30);//box type
	msg_back_data[4]=box_back_data[7]-0x30;
	msg_back_data[4]=msg_back_data[4]*0x10+(box_back_data[8]-0x30);//box version
...
int Process::param_set()
{
	...
	channel=(msg_data[3]/0x10)*10;
	channel=channel+(msg_data[3]%0x10);
	channelH=3+2*channel;
	channelL=4+2*channel;
	printf("process channel:%d, channelH:%d, channelL:%d\n",channel,channelH,channelL);
	...
	box_data[channelH]=msg_data[4]/0x10+0x30;//param H
	box_data[channelL]=msg_data[4]%0x10+0x30;//param L
	...
	if((box_back_data[channelH-1]==box_data[channelH])&(box_back_data[channelL-1]==box_data[channelL]))
		...
		printf("FAIL:process param set failed\n");
		}
